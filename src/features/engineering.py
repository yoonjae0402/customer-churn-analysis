...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        df["tenure_bins"] = (
            pd.cut(df["tenure"], bins=8, labels=False, right=False)
            .fillna(0)
            .astype(int)
        )
        ...
        df["avg_monthly_spend"] = df["TotalCharges"] / (
        ...
        df["price_increase"] = (df["MonthlyCharges"] > df["avg_monthly_spend"]).astype(
            int
        )
        df["high_monthly_charge"] = (df["MonthlyCharges"] > self.monthly_median).astype(
            int
        )
        ...
        df["has_auto_payment"] = (
            df["PaymentMethod"].isin(self.auto_payment_methods).astype(int)
        )
