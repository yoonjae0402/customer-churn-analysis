...
class MarketingService:
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.warning(
                "Gemini API key not configured. Marketing offers will not be generated."
            )
            self.client = None
        else:
    ...
    async def generate_offer(
        self, churn_probability: float, customer_data: Dict[str, Any]
    ) -> str:
        ...
        try:
            senior_citizen_status = (
                "Yes" if customer_data.get("SeniorCitizen") == 1 else "No"
            )
            ...
            Based on these details, please generate a personalized marketing offer aimed
            at retaining this customer. Consider their services, contract type, charges,
            and churn probability. The offer should be concise, persuasive, and suggest
            specific actions or benefits. If the churn probability is high (e.g., above
            50%), focus on strong retention offers. If the churn probability is low,
            perhaps suggest value-added services or loyalty programs.
