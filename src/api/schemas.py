...
class CustomerData(BaseModel):
    ...
    Partner: Literal["Yes", "No"] = Field(..., description="Does customer have a partner?")
    Dependents: Literal["Yes", "No"] = Field(..., description="Does customer have dependents?")
    tenure: int = Field(..., ge=0, description="Months customer has stayed with the company")
    PhoneService: Literal["Yes", "No"] = Field(..., description="Does customer have phone service?")
    ...
    MonthlyCharges: float = Field(..., gt=0, description="Customer's monthly charges in USD")
    TotalCharges: Optional[float] = Field(
        None, ge=0, description="Customer's total charges (computed from tenure if omitted)"
    )
...
class PredictionResponse(BaseModel):
    ...
    churn_prediction: int = Field(
        ..., description="Binary churn prediction at optimal threshold (1=churn, 0=no churn)"
    )
    marketing_offer: str = Field(..., description="AI-generated personalised retention offer")
    metadata: dict = Field(..., description="model_version, latency_ms, model_type, threshold")
